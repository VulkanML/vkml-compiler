...
# vkml-mlir vkml Dialect. This is performed by calling
# `make OMvkmlOpsIncTranslation` in the build dir.
# If the changes are not seen, then you need to rebuild the entire vkml-mlir.
...
for schema in defs.get_all_schemas_with_history():
    if schema.name not in version_dict:
        version_dict[schema.name] = []
    version_dict[schema.name].append(schema.since_version)
...
# Get this order from TensorProto in https://github.com/vkml/vkml/blob/main/vkml/vkml.in.proto#L481.
# enum DataType {
#     UNDEFINED = 0;
#     // Basic types.
#     FLOAT = 1;   // float
...
# the mapping method. MAX_NUM_TYPES should be greater than the length of vkml_types
...
# vkml allows input and output to have the same name. But MLIR does not
...
def vkml_attr_type_to_mlir_attr_type(t):
    vkml_attr_type = Text(t)
    vkml_attr_type = vkml_attr_type[vkml_attr_type.rfind(".") + 1 :].lower()

    if vkml_attr_type == "int":
        mlir_attr_type = "SI64Attr"
    elif vkml_attr_type == "float":
        mlir_attr_type = "F32Attr"
    elif vkml_attr_type == "ints":
        mlir_attr_type = "I64ArrayAttr"
    elif vkml_attr_type == "floats":
        mlir_attr_type = "F32ArrayAttr"
    elif vkml_attr_type == "string":
        mlir_attr_type = "StrAttr"
    elif vkml_attr_type == "strings":
        mlir_attr_type = "StrArrayAttr"
    elif vkml_attr_type in {"type", "type_proto"}:
...
def np_type_to_tblgen_attr_type(tstr):
    index = -1
    for i in range(len(vkml_types)):
        if vkml_types[i] in tstr:
            index = i
            break
    if index == -1:
        return None
...
def get_operands_or_results(schema, type_str_dict, op_name, is_input):
    ...
    for i, value in enumerate(value_list):
        str_types = get_vkml_mlir_types(schema, type_str_dict, value)
...
def get_attrs(schema):
    def get_attr_type_optional(attr_type):
        return "OptionalAttr<{}>".format(vkml_attr_type_to_mlir_attr_type(attr_type))
    ...
    def get_attr_type_with_default(attr_type, attr_default):
        if attr_type == OpSchema.AttrType.STRING:
            return 'DefaultValuedStrAttr<{}, "{}">'.format(
                vkml_attr_type_to_mlir_attr_type(attr_type), attr_default
            )
        else:
    ...
    for _, attr in sorted(schema.attributes.items()):
        ...
        if qualified_attr_name in special_attr_types:
            name_to_type[attr.name] = vkml_attr_type_to_mlir_attr_type(
                special_attr_types[qualified_attr_name]
            )
        # Option holds either required or default value.
        elif attr.required:
            name_to_type[attr.name] = vkml_attr_type_to_mlir_attr_type(attr.type)
...
def parse_type_str(allowedType):
    # AnyI may be used for uint because the vkml_mlir is not generating uint output.
    # This will be fixed later and UI will be replace AnyI.
    vkml_to_mlir_type_dict = {
        "(": "<[",
        ")": "]>",
        "tensor": "TensorOf",
        "seq": "SeqOf",
    ...
    mapping = list(vkml_to_mlir_type_dict.items())
...
def parse_a_type_constraint(constraint):
    ...
    # However vkml keeps a consistently meaningful order
    # There is no redundancy as long as each vkml type is mapped uniquely
...
def get_vkml_mlir_types(schema, type_str_dict, input):
    if input.type_str:
...
def gen_shape_helper_code(s, indent, opName):
    ...
    s += (
        indent
        + "vkml_mlir::vkmlOpShapeHelper * $cppClass::getShapeHelper(mlir::Operation *op, llvm::ArrayRef<mlir::Value> oper, \n"
    )
    indent = inc_indent(indent)
    indent = inc_indent(indent)
    s += (
        indent
        + "vkml_mlir::IndexExprBuilder *ieb, vkml_mlir::IndexExprScope *scope) {\n"
    )
    indent = dec_indent(indent)
    s += (
        indent
        + "vkml_mlir::vkmlOpShapeHelper *sh = new vkml_mlir::vkml{0}OpShapeHelper(op, oper, ieb, scope);\n".format(
            opName
        )
    )
...
def gen_op_def(schema, with_version=False):
    ...
    # Dummy implementations are added to vkmlOps.cpp.
    ...
    if len(regions):
        traits.append('OpInterface<"HasvkmlSubgraphOpInterface">')
    ...
    s += indent + 'let summary = "vkml {} operation";\n'.format(schema.name)
    ...
    # Parse type constraint into vkml-mlir type string list.
...
def gen_op_importer(schema, file, with_version=False):
    ...
    if with_version:
        handler_func = "buildOperation<mlir::vkml{}Op>".format(opName)
    else:
        handler_func = special_op_handler.get(
            schema.name, "buildOperation<mlir::vkml{}Op>".format(opName)
        )
    ...
    """
    s += inc_indent(indent) + "&vkml_mlir::detail::FrontendGenImpl::"
    s += handler_func + ";\n"
    ...
    for domain, _support_map in sorted(index.items()):
        ...
        for _support, _name_map in sorted(_support_map.items()):
            ...
            for n, unsorted_versions in sorted(_name_map.items()):
                ...
                if not found:
                    print(
                        "Your vkml installation may be too old. "
...
def main(args):  # type: (Type[Args]) -> None
    ...
    autogen_warning = (
        ...
        "//   Details can be found in docs/ImportvkmlDefs.md .\n"
        "//********************************************************\n\n"
    )
...
if __name__ == "__main__":
    ...
    class Args(object):
        op_def_file_path = os.path.join(curr_dir, "vkmlOps.td.inc")
        op_header_file_path = os.path.join(curr_dir, "vkmlOps.h")
        
        op_def = io.open(op_def_file_path, "w", newline="")
        op_importer_file_path = os.path.join(curr_dir, "vkmlOpBuildTable.inc")
